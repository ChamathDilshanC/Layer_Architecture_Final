...
public class EventFormController {
    ...
    private void loadEventNames() {
        try {
            eventMap = reservationBO.getWeddingEvents();
    ...
    private void generateNextEventId() {
        try {
            String nextEventId = eventBO.getEventNextId();
    ...
    void btnPlaceEvent(ActionEvent event) throws SQLException {
        ...
        EventDTO ev = new EventDTO(eventId, reservationId, eventName, eventDate, numberOfAttendees, customerPhone, hallNumber);
        ReservationDTO reservation = new ReservationDTO(reservationId,eventDate ,eventDate , eventDate, "1 Day", numberOfAttendees, "Wedding");
        PaymentDTO payment = new PaymentDTO(lablenextpaymentid.getText(), lableReservationId.getText(), total, datepikerEventDate.getValue(), cmbPaymentMethod.getValue().toString(), cmbStatus.getValue().toString());

        boolean isSaved = eventBO.saveEvent(ev, reservation, payment);
