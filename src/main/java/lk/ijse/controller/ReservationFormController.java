...
import lk.ijse.dto.tm.ReservationCartItemDTO;
...
public class ReservationFormController {
    ...
    @FXML
    private TableView<ReservationCartItemDTO> tblreservationcart;
    @FXML
    private TableColumn<ReservationCartItemDTO, String> colReservationId, colPackageId, colServiceId, colRoomID;
    @FXML
    private TableColumn<ReservationCartItemDTO, Double> colPackagePrice, colServicePrice, colTotalPrice;
    @FXML
    private TableColumn<ReservationCartItemDTO, String> colCustomerId;
    @FXML
    private TableColumn<ReservationCartItemDTO, Void> colAction;
    ...
    private void setNetTotal() {
        double netTotal = tblreservationcart.getItems().stream()
                .mapToDouble(ReservationCartItemDTO::getTotalPrice)
                .sum();
    ...
    private void loadRoomIds() {
        ...
        try {
            roomIds.addAll(roomBO.getRoomIds());
    ...
    private void setupRemoveButtonColumn() {
        colAction.setCellFactory(param -> new TableCell<ReservationCartItemDTO, Void>() {
            private final Button btn = new Button("Remove");

            {
                btn.setOnAction((ActionEvent event) -> {
                    ReservationCartItemDTO item = getTableView().getItems().get(getIndex());
    ...
    private void fillServiceComboBox() throws SQLException {
        ObservableList<String> serviceIds = null;
        try {
            serviceIds = FXCollections.observableArrayList(serviceBO.getServiceIds());
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(e);
        }
    ...
    private void getPackageIds() {
        ...
        try {
            List<String> idList = packageBO.getPackageIds();
    ...
    void btnAddToCartOnAction(ActionEvent event) {
        if (areFieldsValid()) {
            ...
            setNetTotal();
            ReservationCartItemDTO item = new ReservationCartItemDTO(
    ...
    void cmbCustomerOnAction(ActionEvent event) {
        ...
        if (selectedCustomerId != null) {
            try {
                CustomerDTO customer = customerBO.searchCustomerById(selectedCustomerId);
                if (customer != null) {
    ...
    void cmbpackageidOnAction(ActionEvent event) {
        ...
        if (selectedPackageId != null) {
            try {
                PackageDTO packageObj = packageBO.searchPackageById(selectedPackageId);
                if (packageObj != null) {
    ...
    private void getCurrentreservationId() {
        try {
            String currentId = reservationBO.getCurrentReservationId();
            String nextOrderId = reservationBO.getNextIdReservation();
    ...
    void cmbserviceidOnAction(ActionEvent event) {
        ...
        if (selectedServiceId != null) {
            try {
                double priceUSD = serviceBO.getServicePriceById(selectedServiceId);
    ...
    void cmbroomidOnAction(ActionEvent actionEvent) {
        ...
        if (selectedRoomId != null) {
            try {
                String status = roomBO.getRoomStatus(selectedRoomId);
    ...
    void btnPlaceReservationOnAction(ActionEvent event) throws SQLException {
        ...
        List<ReservationpackageinfoDTO> reservationpackgagelist = new ArrayList<>();

        for (ReservationCartItemDTO item : tblreservationcart.getItems()) {
            ReservationpackageinfoDTO reservationpackageinfo = new ReservationpackageinfoDTO(
                    reservationid,
                    item.getPackageId(),
                    item.getCustomerId()
            );
        ...
        List<ReservationserviceinfoDTO> reservationservicelist = new ArrayList<>();

        for (ReservationCartItemDTO item : tblreservationcart.getItems()) {
            ReservationserviceinfoDTO reservationserviceinfo = new ReservationserviceinfoDTO(
                    reservationid,
                    item.getServiceId()
            );
        ...
        List<ReservationroominfoDTO> reservationroominfoList = new ArrayList<>();
        for (ReservationCartItemDTO item : tblreservationcart.getItems()) {
            ReservationroominfoDTO reservationroominfo = new ReservationroominfoDTO(
                    reservationid,
                    item.getRoomID()
            );
        ...
        PlaceReservationDTO placereservation = new PlaceReservationDTO(reservation, reservationpackgagelist, reservationroominfoList, reservationservicelist, payment);

        boolean isPlaced = reservationBO.placereservation(placereservation);
        if (isPlaced) {
